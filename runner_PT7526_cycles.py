...
profile_parent_folder = "G:/My Drive/Cell Test Profiles/Cycles/PT7526_tuned_profiles"
savepath = "G:/My Drive/Cell Test Data/PT7526/Cycles"
test_title = 'PT7526_Cycles'
...
# Storage charge (run after cycles complete)
strg_profile = "G:/My Drive/Cell Test Profiles/Utilities/storage_charge_H52.xml"
strg_savepath = "G:/My Drive/Cell Test Data/PT7526/Storage_Charges"
strg_temp = 20
...
# prompt user for which bank to run (single bank for now)
...
# the above only passes if all cells reach the "finish" state within the timeout, otherwise the program raises exception and exits
...
while cycles_completed < cycles_to_complete:
    ...
    for specimen in specimens:
        ...
        discharge_type = cycle_manager.get_discharge_type(current_cycle, specimen)    # pulls from the cycle lookup table to see if it is extended or standard
...
message = f'''{cycles_to_complete} cycles completed successfully for cycle accumulation of PT-7526. All data saved to {savepath}.
