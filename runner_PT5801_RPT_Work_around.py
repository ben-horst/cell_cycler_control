...
from Work_around import TestRunner
...
for bank in active_banks:
    all_channels.extend(CONFIG.CHANNELS_PER_BANK[bank])
...
test_runner = TestRunner(all_channels, test_title, ignored_banks=ignored_banks)
barcodes = test_runner.barcodes

for temp in temps:
    test_runner.bring_all_cells_to_temp_and_block_until_complete(temp=temp, timeout_mins=60)
    test_runner.start_tests(all_channels, profile, savepath, filenames)
